        ...
        <script>
            function loadChats(snapshot) {
                ...
                snapshot.forEach(function(doc) {
                    ...
                    listarr.indexOf(message.message.from) === -1 && message.message.type == 0 ? $('#scrollable').append(`<div class="message_layout" onclick=showMessages(${message.message.from})>
                    <div class="message_main">
                    <div class="user_img small"></div>
                    <div class="message_data">
                    <div class="sender">${message.message.from}</div>
                    <div class="message">
                    ${message.message.message}
                    </div>
                    <div class="message_footer">
                    <div class="message_to">${message.message.to}</div>
                    <div class="date">July 19,2023 13:43</div>
                    </div>
                    </div>
                    </div>
            ...
            function showMessages(number) {
                ...
                window.snapshot.forEach(function(doc) {
                    var message = doc.data();
                    console.log(message)
                    if (parseInt(message.message.from) == number) {
                        if (parseInt(message.message.type) == 1) {
            ...
            firebase.auth().onAuthStateChanged((user) => {
                if (user) {
                    var msg = recieveMessage(user.uid).then(function(response) {
